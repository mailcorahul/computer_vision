...
def vertical_segmentation(img , crop , char , img_flag ) :

	segm_chars = [] ;
	vert_proj = np.sum(crop , axis=0) ;
	...
	for i in range(vert_proj.shape[0]) :
		...
		if vert_proj[i] == max_col_val : #and ( st + i - 1 ) - st > 0 :
			if is_black and i - 1 > j:#and st + i - 1 <= char[2] :
				#print 'in' ;
				segm_chars.append([st + j , char[1] , st + i - 1 , char[3]]) ;
				j = st + i + 1 ;
				#print i , st ;
				is_black = False ;
	...
	if st < char[2] and is_black :
		#print 'else' ;
		segm_chars.append([st + j , char[1] , char[2] , char[3]]) ;

	return segm_chars ;


def filters(img ,rect , path , index) :
	...
	for k , v in char_cont.items() :
		...
		'''
		crop = img[rect[k][1] : rect[k][3] , rect[k][0] : rect[k][2]] ;
		crop = cv2.cvtColor(crop ,cv2.COLOR_BGR2GRAY) ;
		ret,crop = cv2.threshold(crop,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU) ;
		#cv2.imwrite(path + '/ocr/' + str(index) + '_' + str(k) + '.jpeg', crop) ;
		...
		if is_same_height :
			...
			# else :
			segmented_chars = vertical_segmentation(img ,crop , rect[k] , img_flag) ;
