  ...
  for row in xrange(cols):
    for col in xrange(rows):
      ...
      while True:
        ...
        next_row = next_row+1
   '''
  pos = np.where(horizontal==1)
  diff = np.diff(pos[1])
  
  p =  np.where((diff>1)&(diff<h_threshold))[0]
  
  for i in p:
      horizontal[pos[0][i],pos[1][i]+1:pos[1][i+1]] = 1
...
def sortMSERs(img , lines , msers) :
  ...
  for line in lines :
    ...
    for pts in msers :
      ...
      if pts[3] - pts[1] > 0 and pts[2] - pts[0] > 0 and np.amin(crop) == 0 :
